...
def visualize(sess, model, config, option):
  ...
  if option == 0:
    ...
    samples = sess.run(model.sampler, feed_dict={model.z: z_sample})
    ...
    for idx in xrange(100):
      ...
      if config.dataset == "mnist":
        ...
        samples = sess.run(model.sampler, feed_dict={model.z: z_sample, model.y: y_one_hot})
      else:
        samples = sess.run(model.sampler, feed_dict={model.z: z_sample})
    ...
    for idx in xrange(100):
      ...
      image_set.append(sess.run(model.sampler, feed_dict={model.z: z_sample}))
